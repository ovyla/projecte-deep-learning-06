...
from src.shared.dataset import (
    ...
    split_image_ids,
    load_captions_df,
)  # els dataloaders i la funció per dividir el dataset
...
)  # vocabulari, tokenitzador, GloVe i Word2Vec
...
def parse_args():  # a llegir tots els arguments
    ...
    )  # directori on es guarden els checkpoints de l'entrenament (epoch, vocab_size, args...) i models entrenats
    p.add_argument(
        "--vocab-threshold", type=int, default=5
    )  # mínim de vegades ha d'aparèixer una paraula per entrar al vocabulari (default 5, si no entra --> <unk>)
    ...
    )  # probabilitat de dropout a la LSTM (regularització que apaga neurones aleatòriament durant l'entrenament per evitar overfitting)
    p.add_argument(
        "--backbone", default="resnet152"
    )  # quina CNN preentrenada utilitzar com a encoder (resnet50 o resnet152)

    p.add_argument("--epochs", type=int, default=20)  # numero de passades completes del train
    p.add_argument(
        "--patience", type=int, default=999
    )  # nombre d'epochs que esperarem sense millora en la val_loss abans de parar l'entrenament (early stopping)
    ...
    )  # de l'optimitzador Adam (la mida del pas d'actualització dels pesos)
    ...
    )  # nom de l'entitat (usuari o organització) a wandb, si es deixa None s'utilitzarà l'entitat per defecte de l'usuari
    p.add_argument(
        "--run-name", default=None
    )  # nom l'execució concreta, si es deixa None s'utilitzarà un nom generat automàticament basat en la data i hora actual
    return p.parse_args()
...
def get_or_build_vocab(args) -> Vocabulary:  # funció que retorna l'objecte Vocabulary
    ...
    vocab = build_vocab(
        args.captions_csv, threshold=args.vocab_threshold
    )  # si no existeix, el construeix a partir de les captions i threshold
...
def main():
    ...
    )  # entrena amb gpu si està disponible, si no cpu
    ...
    print(
        f"[data] train batches={len(train_loader)}  val batches={len(val_loader)}"
    )  # mira quants batches hi ha al train i a la validació
    ...
    params = (  # llista de paràmetres que entrenarem
        ...
        )  # inclou els de l'encoder (només s'entrenaven les capes linear i bn, les altres es congelaven)
        + list(encoder.bn.parameters())
    )
    optimizer = torch.optim.Adam(
        params, lr=args.lr
    )  # crea l'optimitzador Adam. Només actualitzarà els paràmetres especificats a params, i li especifiquem la learning rate.
    ...
    if use_wandb:  # si hem activat wandb
        ...
        wandb.init(
            project=args.wandb_project, entity=args.wandb_entity, name=args.run_name, config=vars(args)
        )  # li passem els arguments
        wandb.config.update(
            {"vocab_size": len(vocab), "embedding_type": emb_type}
        )  # afegeix mida del vocabulari i tipus d'embedding a wandb
    ...
    for epoch in range(1, args.epochs + 1):  # bucle d'epochs
        ...
        for i, (images, captions, lengths) in enumerate(
            train_loader
        ):  # recorre tots els batches d'entrenament; i --> index del batch, els altres són les dades del batch
            images = images.to(
                device, non_blocking=True
            )  # mou les imatges a gpu o cpu. non_blocking pot accelerar la transferència si el DataLoader utilitza memòria pinned (que ho fa)
            captions = captions.to(device, non_blocking=True)  # mou les captions al dispositiu
            targets = pack_padded_sequence(
                captions, lengths, batch_first=True
            ).data  # crea els objectius reals del model eliminant padding (perquè el model aprengui a predir <pad>)

            features = encoder(images)  # passa les imatges per la CNN
            outputs = decoder(
                features, captions, lengths
            )  # passa els embeddings de les imgs, les captions i les lenghts a la LSTM
            # per cada token dona una distribució sobre el vocabulari
            loss = criterion(
                outputs, targets
            )  # calcula la Cross Entropy Loss comparant les prediccions del model amb les paraules correctes
            ...
            if i % args.log_step == 0:  # comprova si toca imprimir la informació
                ...
                with torch.no_grad():
                    ...
                    p_preds = PackedSequence(
                        preds_idx, targets.batch_sizes, targets.sorted_indices, targets.unsorted_indices
                    )
                    p_targets = PackedSequence(
                        targets, targets.batch_sizes, targets.sorted_indices, targets.unsorted_indices
                    )
                ...
                print(
                    ...
                    f"loss={loss.item():.4f}  ppl={ppl:.2f}  b1={batch_b1:.3f}  b4={batch_b4:.3f}  meteor={batch_m:.3f}"
                )  # imprimeix info de l'entrenament
        ...
        )  # validació després de cada epoch, que és la loss mirjana de validació
        ...
        if val_loss < best_val_loss:  # comprova si la validació ha millorat
            ...
            print(
                f"[best] new best val_loss={best_val_loss:.4f} -> saved ckpt_best.pt"
            )  # mostra que hi ha un nou millor model
    ...
    axes[1].plot(
        range(1, len(val_losses) + 1), val_losses, marker="o", label="val"
    )  # la de la dreta és la gràfica de validació
    ...
    for img in test_ids:
        ...
        if bleu_table is not None and len(bleu_table.data) < TABLE_LIMIT:
            ...
            if args.flickr30k_hf:
                ...
                bleu_table.add_data(
                    wandb.Image(pil_img), " ".join(hyp), ref_str, round(b1, 3), round(b4, 3), round(m, 3)
                )
    ...
    if use_wandb:
        wandb.log(
            {"test/eval_table": bleu_table, "test/corpus_bleu1": cb1, "test/corpus_bleu4": cb4, "test/meteor": cm}
        )
    # ------------------------------------------------
