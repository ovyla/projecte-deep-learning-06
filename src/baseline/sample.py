...
def load_checkpoint(ckpt_path: str, vocab_path: str, device: torch.device):
    ...
    encoder = (
        EncoderCNN(a["embed_size"], backbone=a["backbone"]).to(device).eval()
    )  # reconstrueix l'encoder i es posa en mode eval
    decoder = (
        DecoderRNN(a["embed_size"], a["hidden_size"], len(vocab), a["num_layers"]).to(device).eval()  # i el decoder
    )  # i mode eval
...
def caption_image(image_path: str, encoder, decoder, vocab, device) -> str:
    ...
    ids = (
        decoder.sample(feat).cpu().numpy()[0].tolist()
    )  # crea la caption (lista d'ids) del vector de característiques passant-lo pel decoder
...
def main():
    ...
    encoder, decoder, vocab = load_checkpoint(
        args.checkpoint, args.vocab, device
    )  # funció load_chechpoint per crear el model
